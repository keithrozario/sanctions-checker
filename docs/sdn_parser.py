"""
This file was taken from https://github.com/archerimpact/SanctionsExplorer/blob/master/data/sdn_parser.py
It already contains the logic to parse the xml file
"""
...
def days_span_month(from_date, to_date):
    ...
    to_ends = (
        to_date.day == "31" or to_date.day == "30"
    )  # crude, but works for now.  TODO make a proper mapping of month -> num_days
...
class DateBoundary:
    ...
    def condense_boundary(self, from_date, to_date):
        if str(from_date) == str(to_date):
            ...
            years_match(from_date, to_date)
            and months_span_year(from_date, to_date)
            and days_span_month(from_date, to_date)
        ):
            ...
            log(
                "DateBoundary was not condensable: "
                + str(from_date)
                + ", "
                + str(to_date),
                "error",
...
class DatePeriod:
    ...
    def condense_and_stringify(self, from_boundary, to_boundary):
        if str(from_boundary) == str(to_boundary):
            ...
            years_match(from_boundary, to_boundary)
            and months_span_year(from_boundary, to_boundary)
            and days_span_month(from_boundary, to_boundary)
        ):
            # these dates span one year, so we will use the year they span
            ...
            years_match(from_boundary, to_boundary)
            and months_match(from_boundary, to_boundary)
            and days_span_month(from_boundary, to_boundary)
        ):
            ...
            (not years_match(from_boundary, to_boundary))
            and months_span_year(from_boundary, to_boundary)
            and days_span_month(from_boundary, to_boundary)
        ):
            ...
            from_boundary.year is not None
            and to_boundary.year is not None
            and from_boundary.month is None
            and to_boundary.month is None
            and from_boundary.day is None
            and to_boundary.day is None
        ):
            ...
            log(
                "A DatePeriod did not condense nicely: "
                + str(from_boundary)
                + " to "
                + str(to_boundary),
                "warning",
    ...
    def __init__(self, xml):
        ...
        self.start_boundary = self.parse_date_boundary(
            xml, "Start"
        )  # a DateBoundary Object
        self.end_boundary = self.parse_date_boundary(
            xml, "End"
        )  # a DateBoundary Object
        ...
        if (
            ...
            or self.month_fixed != "false"
            or self.day_fixed != "false"
        ):
            log("OFAC is beginning to use year_/month_/day_fixed fields", "warning")

    def __str__(self):
        return self.condense_and_stringify(
            self.start_boundary.condensed_date, self.end_boundary.condensed_date
        )
...
class Location:
    ...
    def __init__(self, xml):
        ...
        self.country = self.parse_country(
            xml
        )  # country object, lol assumes it exists, will break everything if it doesn't
        self.location_parts = self.parse_location_parts(
            xml
        )  # dict map (str locparttype, str locpart)
        self.feature_version_ids = self.parse_feature_version_ids(
            xml
        )  # list of feature versions ids that must be evaluated later
        self.id_reg_doc_ids = self.parse_reg_doc_ids(
            xml
        )  # list of reg_doc_ids that must be evaluated later
        self.area_code = str(self.parse_area_code(xml))

        # Checks that there are not conflicting country entries in this location
        if (
            (self.country is not None)
            and (self.country != "None")
            and (str(self.location_parts.get("COUNTRY")) != "None")
        ):
            log(
                str(self.country)
                + " and "
                + self.location_parts["COUNTRY"]
                + " are listed as countries for the same location",
                "warning",
            )
    ...
    def __str__(self):
        ...
        order = [
            ...
            "COUNTRY",
            "Unknown",
        ]  # note: if the key is 'Unknown', it will be the only key in the dict

        # Checks whether OFAC has added any new fields
        if len(set(self.location_parts.keys() - set(order))) > 0:
            log(
                "OFAC has added a new location part type: "
                + str(self.location_parts.keys()),
                "warning",
            )
...
class Feature:
    ...
    def __init__(self, feature_xml):
        ...
        self.comment = xml_approx_find(
            self.feature_version, "Comment"
        ).text  # should get
    ...
    def __str__(self):
        ...
        d["comment"] = (
            self.comment
        )  # there's only a few of these, and they're probably being phased out because they're horrible.
...
class Alias:
    ...
    def construct_name_string(self, name_dict):
        unknown_keys = set(name_dict.keys()) - set(
            [
                "Entity Name",
                "Aircraft Name",
                "Vessel Name",
    ...
    def __init__(self, xml, name_part_groups_dict):
        ...
        self.documented_name = self.parse_documented_names(
            xml, name_part_groups_dict
        )  # A list of (nameparttype, namepart) tups
        ...
        if type(self.documented_name) == type([]):
            ...
            self.documented_name = self.documented_name[
                "main_name"
            ]  # NOTE: This must come after we set non_english_names, since we're overwriting the variable main_name
...
class Identity:
    ...
    def parse_name_part_groups(self, identity_xml):
        ...
        for e in elems:
            ...
            ret[np_group.get("ID")] = name_part_types[
                np_group.get("NamePartTypeID")
            ].text
        return ret
    ...
    def __init__(self, xml):
        ...
        self.name_part_groups = self.parse_name_part_groups(
            xml
        )  # mapping from ID : NamePartTypeID
        ...
        self.all_names = self.parse_aliases(
            xml, self.name_part_groups
        )  # consists of one primary and some number of aliases
        self.aliases = []
        self.primary = None
        ...
        for a in self.all_names:
            if a.non_english_names != []:
                ...
                for foreign in a.non_english_names:
                    ...
                    newAlias.is_primary = (
                        False  # their English name will remain primary if applicable
                    )
                    newAlias.alias_type = (
                        "A.K.A."  # their non-English name will be an A.K.A.
                    )
    ...
    def parse_sanctions_measures(self, sanction_entry_xml):
        ...
        if measures is not None:
            for m in measures:
                ...
                ):  # currently, sanctions block doesn't contain anything but a date.
                    ret.append(comment)

                if sanctions_type == "Block" and comment is not None:
                    log(
                        "Sanctions Block now contains a comment (" + comment + ")",
                        "warning",
                    )
...
class ProfileLink:
    ...
    def __init__(self, xml, is_reverse):
        ...
        if is_reverse:
            ...
            self.linked_profile_id = xml.get(
                "To-ProfileID"
            )  # we'll keep this as a string for now.  We'll serialize it as a number in the __str__ method.  TODO come back and look at this.
...
class DistinctParty:
    def parse_comment(self, xml):
        ...
        if elem is not None and elem.text is not None:
            return [
                x.strip() for x in list(filter(None, elem.text.split(";")))
            ]  # they use ;-separated comments.  filter to remove emtpy string, strip whitespaces on either side.  is this what Pythonic means?
        else:
            return None
    ...
    def __init__(self, xml):
        ...
        self.party_sub_type = party_sub_types[
            self.profile.get("PartySubTypeID")
        ].text  # read subtypeid from profile and fetch value
        self.identity = Identity(
            xml_approx_find(self.profile, "Identity")
        )  # OFAC doc says one identity
        self.features = self.parse_features(self.profile)  # any number of features
        self.sanctions_entry_reference_ids = self.parse_sanctions_entry_ids(
            self.profile
        )
